...
gh api "repos/mhenrixon/glyphs/pulls/<PR_NUMBER>/comments" --paginate
...
' -f owner=mhenrixon -f repo=glyphs -F pr=<PR_NUMBER>
```
...
| Valid test gap | Add the missing spec |
...
Grounding for glyphs when evaluating comments:
- Icon components (`lib/glyphs/icon.rb`, the per-library subclasses, `svg_for` resolution
  in `lib/glyphs.rb`) render SVGs from `app/assets/svg/icons/<library>/<variant>/<name>.svg`
  via the transitive `icons` gem -- a suggestion that ignores that resolution path is wrong.
- Pruning changes (`source_scanner.rb`, `icon_pruner.rb`, `prune_runner.rb`) must preserve
  the AST + template-text scan and `keep_icons`/`prune_source_globs` config semantics --
  reject suggestions that would drop dynamically-referenced icons.
- RuboCop cop changes (`lib/rubocop/cop/glyphs/*.rb`) should keep offense/autocorrect
  behaviour verifiable via `expect_offense`/`expect_correction`.
...
2. **Run affected specs** to verify nothing breaks:
   ```bash
...
3. **Run validators**:
   ...
   ```
4. **Commit** all fixes together with a clear message (use a scope that fits glyphs:
   `scanner`, `prune`, `config`, `rubocop`, `icon`, `rake`, `docs`, `ci`, `chore`):
   ```bash
...
Never bump the gem version inside a review-feedback commit -- `rake release` owns the version.
...
```bash
gh api "repos/mhenrixon/glyphs/pulls/<PR>/comments/<COMMENT_ID>/replies" \
  --method POST \
